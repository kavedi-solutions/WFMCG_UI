...
@Component({
  selector: 'app-vcontra',
  templateUrl: './vcontra.component.html',
  styleUrls: ['./vcontra.component.scss'],
})
export class VContraComponent implements OnInit {
  PageTitle: string = 'Contra Voucher';
  buttonText: string = 'Add New Contra';
  voucherListData: VContra[] = [];
          ...
          children: [
            ...
            },
            {
              ...
              pop: {
                title: 'Confirm Print',
                description:
                  'Are you sure you want to Print this Contra Voucher.',
                closeText: 'No',
                okText: 'Yes',
                okColor: 'primary',
                closeColor: 'warn',
  ...
  edit(value: any) {
    this.router.navigate(['/transaction/v-contra/edit/', value.autoID]);
  }
  ...
  printVoucher(value: any) {
    this.reportService
      .PrintVouchers('CV', 0, [value.autoID])
      .subscribe((response) => {
  ...
  AddnewRecord() {
    this.router.navigate(['/transaction/v-contra/add']);
  }
