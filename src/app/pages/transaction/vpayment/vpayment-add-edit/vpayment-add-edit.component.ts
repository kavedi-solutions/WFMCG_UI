...
@Component({
  selector: 'app-vpayment-add-edit',
  templateUrl: './vpayment-add-edit.component.html',
  styleUrls: ['./vpayment-add-edit.component.scss'],
})
export class VPaymentAddEditComponent implements OnInit {
  PageTitle: string = 'Create Payment Voucher';
  buttonText: string = 'Add New Payment Voucher';
  isEditMode: boolean = false;
  isFromQuickMenu: boolean = false;
  selectedVoucherId: number;

  voucherPostRequest?: VPaymentPostRequest;
  voucherPutRequest?: VPaymentPutRequest;
  editVoucher?: VPaymentResponse;
  ...
  BacktoList() {
    if (this.isFromQuickMenu == false) {
      this.router.navigate(['/transaction/v-payment/list']);
    } else {
