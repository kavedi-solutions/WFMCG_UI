...
import {
  ...
  TransferDumpItemDetail,
  TransferDumpItemPostRequest,
  TransferDumpItemPutRequest,
  TransferDumpPostRequest,
  TransferDumpPutRequest,
  TransferDumpResponse,
} from 'src/app/shared';
...
@Component({
  selector: 'app-transfer-dump-add-edit',
  templateUrl: './transfer-dump-add-edit.component.html',
  styleUrls: ['./transfer-dump-add-edit.component.scss'],
})
export class TransferDumpAddEditComponent implements OnInit {
  PageTitle: string = 'Create Transfer to Dump';
  buttonText: string = 'Add New Transfer to Dump';
  ...
  ItemEdit?: TransferDumpItemDetail;
  IsItemEditMode: boolean = false;
  ItemCount: number = 0;

  transferItemDetailsList: TransferDumpItemDetail[] = [];
  transferItemDetailsListData: TransferDumpItemDetail[] = [];

  columns: MtxGridColumn[] = [];

  editTransfer?: TransferDumpResponse;
  ...
  ngOnInit(): void {
    ...
    if (this.selectedTransferId != 0) {
      ...
      this.PageTitle = 'Update Transfer to Dump';
  ...
  setColumns() {
    this.columns = defaultData.GetTransferDumpDetailColumns();
    this.columns.push({
      header: 'Action',
      field: 'action',
      minWidth: 120,
  ...
  BacktoList() {
    if (this.isFromQuickMenu == false) {
      this.router.navigate(['/transaction/transferdump/list']);
    } else {
  ...
  getTransferByID() {
    this.transferDumpService
      .GetTransferDumpbyID(this.selectedTransferId)
      .subscribe((response) => {
        ...
        this.editTransfer!.details!.forEach((element) => {
          let ItemDetails: TransferDumpItemDetail = {
    ...
    this.itemService
      ...
      .subscribe((response) => {
        ...
        if (FoundItem == -1) {
          ...
          let ItemDetail: TransferDumpItemDetail =
  ...
  GetCurrentStock(ItemID: number, EditQty: number) {
    //stockService
    this.stockService.GetClosingByItemID(ItemID, 2).subscribe((response) => {
  ...
  editItem(record: TransferDumpItemDetail) {
  ...
  SaveTransfer(transferForm: FormGroup) {
    let PostRequestDetail: TransferDumpItemPostRequest[] = [];
    ...
    this.transferDumpService
      .createTransferDump(this.transferPostRequest)
      .subscribe((response) => {
        this.BacktoList();
      });
  }
  ...
  UpdateTransfer(transferForm: FormGroup) {
    let PutRequestDetail: TransferDumpItemPutRequest[] = [];
    ...
    this.transferDumpService
      .updateTransferDump(this.editTransfer!.autoID, this.transferPutRequest)
      .subscribe((response) => {
        this.BacktoList();
      });
  }
